...
def enqueue_job_vector_cleanup(job: JobDocument, background_tasks: BackgroundTasks) -> None:
    ...
    background_tasks.add_task(_dispatch_job_vector_cleanup, org_id, job_id, vector_chunk_count)
...
async def _dispatch_job_vector_cleanup(org_id: str, job_id: str, vector_chunk_count: int) -> None:
    if settings.EMBEDDINGS_ASYNC_ENABLED:
        if _try_enqueue_celery_cleanup("delete_job_vectors_task", org_id, job_id, vector_chunk_count):
            return
...
def _try_enqueue_celery_cleanup(task_name: str, *task_args: object) -> bool:
    try:
        ...
        logger.exception("Failed to enqueue vector cleanup task=%s id=%s", task_name, task_args[1])
        return False
