...
async def overview(ctx: dict = Depends(get_auth_context)):
    ...
    return await get_overview_metrics(org_id=ctx["org_id"], profile_id=ctx["profile_id"])
...
async def resume_performance(ctx: dict = Depends(get_auth_context)):
    ...
    return await get_resume_performance(org_id=ctx["org_id"], profile_id=ctx["profile_id"])


@router.get("/analytics/trends")
async def industry_trends(ctx: dict = Depends(get_auth_context)):
    """Tag-based success rate breakdown."""
    return await get_industry_trends(org_id=ctx["org_id"], profile_id=ctx["profile_id"])
